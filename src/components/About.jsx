...
export default function About() {
  ...
  const introBlocks = shouldSplitBio
    ? bioBlocks.slice(0, workIndex)
    : bioBlocks;
                ...
                <div className="mt-8 border-t border-green-700 border-opacity-40 pt-6 w-full min-w-0 overflow-hidden">
                  <div className="flex w-full flex-col gap-3 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
                    ...
                    <div className="flex items-center gap-2 sm:ml-auto">
                      <button
                        type="button"
                        className="rounded-full border border-green-700 border-opacity-60 px-3 py-1 text-sm text-green-100 hover:border-green-400 hover:text-green-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-green-400"
                  ...
                  >
                    {certificates.map((certificate, index) => {
                      ...
                      const isPdf =
                        ...
                        (!certificate.kind && !!pdfUrl && !imageUrl);
                      const thumbnailUrl =
                        certificate.thumbnail?.asset?.url ||
                        (!isPdf ? imageUrl : null);
                      ...
                      return (
                        ...
                        >
                          {thumbnailUrl ? (
                            ...
                            <div className="flex h-24 w-full items-center justify-center rounded-md border border-green-700 border-opacity-40 bg-green-900 bg-opacity-40 text-sm font-semibold text-green-100">
                              {isPdf ? "PDF" : "No preview"}
                            </div>
                          )}
                          <span className="mt-3 w-full truncate text-sm font-semibold text-green-100">
